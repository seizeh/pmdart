...
| | 지역 기반 | 반려동물 특화 | 거래 형태 | 신뢰를 만드는 방식 |
|---|:---:|:---:|---|---|
| **당근 동네생활** | ✅ | ❌ | 이웃 간 P2P | 동네 인증 + 매너온도(범용) |
| **펫프렌즈** | ❌ | ✅ | 커머스(용품 판매) | 판매자=회사 |
| **도그메이트 · 펫트너** | 부분 | ✅ | **회사가 검증한 전문 펫시터 중개** | **회사가 공급자를 심사**(신원·인적성·면접·범죄이력) |
| 맘카페 · 중고거래 게시판 | 부분 | ❌ | 이웃 간 P2P | **없음** |
| **PawMate** | ✅ | ✅ | **이웃 간 P2P** | 지역 인증 + AI 실존 검증 + 상호 후기 |
...
flowchart LR
    ...
    H3 --> S3["지원 → 수락 → 약속 → 상호 후기<br/>업체는 사업자 인증"]
```
...
| **H3** 거래 후 책임이 남아야 한다 | 매칭이 약속·상호 후기까지 이어진다. 업체는 국세청 조회로 사업자를 인증하고, 개인 얼굴과 분리한다 | [ADR-0010](adr/0010-business-two-faces.md) |

세 가설 모두 **클라이언트가 아니라 서버가 강제**한다는 점이 공통이다. P2P 에서는
